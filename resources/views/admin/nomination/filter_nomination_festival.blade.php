...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <h4 class="mb-4">فرز المشاركين في المهرجان </h4>

            <form action="{{ route('filter.users.festival.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    <button type="submit" class="btn btn-success">فرز المشتركين</button>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('#festival_id').on('change', function() {
        ...
        if(festivalId) {
            $.ajax({
                url: '/get-rounds/filter/' + festivalId,
                type: 'GET',
                dataType: 'json',
