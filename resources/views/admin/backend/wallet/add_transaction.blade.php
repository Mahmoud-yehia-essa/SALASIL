...
/* ═════════════════════════════════════════════════════════════
   LIGHT MODE OVERRIDES FOR ADD WALLET TRANSACTION PAGE
═════════════════════════════════════════════════════════════ */
...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold mb-1 header-title">
            <i class="bx bx-plus-circle text-info me-2"></i>Deposit / Withdraw Funds
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Record a deposit, withdrawal, trip earnings, or deduction for a user wallet.
        </p>
    </div>
    <div>
...
<div class="wal-card">
    <div class="wal-card-header">
        <div class="icon-wrap"><i class="bx bx-wallet-alt"></i></div>
        <div>
            <h4 class="fw-bold text-white mb-0">Record Wallet Transaction</h4>
            <small class="text-muted">Select user, operation type, amount and linked order</small>
        </div>
    </div>
    <div class="wal-card-body">
        <form method="POST" action="{{ route('store.wallet.transaction') }}" id="walForm" autocomplete="off">
            @csrf
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-6">
                    ...
                    <select name="user_id" id="user_id" class="form-select">
                        <option value="">-- Select User (Company, Individual, Driver) --</option>
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ (old('user_id') == $u->id || $selectedUserId == $u->id) ? 'selected' : '' }}>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="type" id="type" class="form-select">
                        <option value="deposit" {{ old('type') === 'deposit' ? 'selected' : '' }}>Deposit (+) / إيداع</option>
                        <option value="withdrawal" {{ old('type') === 'withdrawal' ? 'selected' : '' }}>Withdrawal (-) / سحب</option>
                        <option value="trip_earnings" {{ old('type') === 'trip_earnings' ? 'selected' : '' }}>Trip Earnings (+) / أرباح رحلة</option>
                        <option value="commission_deduction" {{ old('type') === 'commission_deduction' ? 'selected' : '' }}>Commission Deduction (-) / خصم عمولة</option>
                        <option value="refund" {{ old('type') === 'refund' ? 'selected' : '' }}>Refund (+) / استرجاع</option>
                    </select>
                </div>
            </div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-6">
                    ...
                    <div class="input-group">
                        <input type="number" step="0.01" name="amount" id="amount" class="form-control" placeholder="0.00" value="{{ old('amount') }}">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" name="description" class="form-control" placeholder="e.g. Deposit for shipment #104 or Trip commission deduction" value="{{ old('description') }}">
                </div>
            </div>
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="scheduled_trip_id" id="scheduled_trip_id" class="form-select">
                        ...
                        @foreach($trips as $tr)
                            <option value="{{ $tr->id }}" {{ old('scheduled_trip_id') == $tr->id ? 'selected' : '' }}>
...
$(document).ready(function() {
    ...
    $('#walForm').on('submit', function() {
        $('#submitBtn').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span> Executing Transaction...');
    });
