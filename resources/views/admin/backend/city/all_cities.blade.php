...
<style>
    ...
    .city-status-select.status-badge-active {
        background-color: rgba(34, 197, 94, 0.18) !important;
        color: #22C55E !important;
        border: 1px solid rgba(34, 197, 94, 0.4) !important;
    }

    .city-status-select.status-badge-inactive {
...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold header-title mb-1">
            <i class="bx bx-building-house text-info me-2"></i>Cities Management
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Manage cities, linked countries, active statuses, and geographic locations.
        </p>
    </div>
    <div>
        <a href="{{ route('add.city') }}" class="btn btn-primary rounded-3 px-3 py-2 fw-semibold d-inline-flex align-items-center gap-2">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="fw-bold text-white fs-6">
                            <i class="bx bx-map-pin me-1 text-info"></i> {{ $item->name_en }}
                        </td>
                        ...
                        <td>
                            @if($item->country)
                                <span class="badge bg-primary bg-opacity-20 text-info border border-info border-opacity-50 px-3 py-1.5 rounded-pill fw-semibold">
                                    <i class="bx bx-flag me-1"></i> {{ $item->country->name_en }} ({{ $item->country->name_ar }})
                                </span>
                            @else
                        ...
                        <td style="min-width: 140px;">
                            <select class="form-select city-status-select status-badge-{{ $item->is_active == 1 ? 'active' : 'inactive' }}" 
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center justify-content-center gap-2">
                                <a href="{{ route('edit.city', $item->id) }}" 
                                ...
                                <a href="{{ route('delete.city', $item->id) }}" 
                                   class="btn btn-sm btn-outline-danger rounded-3 p-2 d-inline-flex align-items-center justify-content-center delete-city-btn"
                                   title="Delete City">
