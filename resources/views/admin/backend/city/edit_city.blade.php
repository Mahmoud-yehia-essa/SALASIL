...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold mb-1 header-title">
            <i class="bx bx-edit-alt text-info me-2"></i>Edit City
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Update city names, country linkage, or active status.
        </p>
    </div>
    <div>
        <a href="{{ route('all.cities') }}" class="btn btn-outline-info rounded-3 px-3 py-2 fw-semibold d-inline-flex align-items-center gap-2">
        ...
        <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="card-header bg-transparent border-bottom border-secondary border-opacity-25 py-3 px-4">
                <h5 class="card-title mb-0 fw-bold d-flex align-items-center gap-2">
                    ...
                    <span>Edit City #{{ $city->id }}</span>
                </h5>
            </div>

            <div class="card-body p-4">
                <form method="POST" action="{{ route('update.city') }}" id="editCityForm">
                    @csrf
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-12">
                            <label for="country_id" class="form-label fw-semibold">Select Country <span class="text-danger">*</span></label>
                            <div class="position-relative">
                                <select name="country_id" id="country_id" class="form-select @error('country_id') is-invalid @enderror" style="padding-left: 48px !important;" required>
                                    <option value="">-- Choose Country --</option>
                                    @foreach($countries as $country)
                                        <option value="{{ $country->id }}" {{ old('country_id', $city->country_id) == $country->id ? 'selected' : '' }}>
                                            {{ $country->name_en }} ({{ $country->name_ar }}) {{ $country->code ? '['.$country->code.']' : '' }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <!-- City Name English (Required) -->
                        <div class="col-12 col-md-6">
                            <label for="name_en" class="form-label fw-semibold">City Name (English) <span class="text-danger">*</span></label>
                            <div class="position-relative">
                                <input type="text" 
                                       name="name_en" 
                                       id="name_en" 
                                       class="form-control @error('name_en') is-invalid @enderror" 
                                       placeholder="e.g. Al Farwaniyah / Riyadh"
                        ...
                        <!-- City Name Arabic (Required) -->
                        <div class="col-12 col-md-6">
                            <label for="name_ar" class="form-label fw-semibold">City Name (Arabic) <span class="text-danger">*</span></label>
                            <div class="position-relative">
                                <input type="text" 
                                       name="name_ar" 
                                       id="name_ar" 
                                       ...
                                       style="direction: rtl;"
                                       placeholder="مثال: الفروانية / الرياض"
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="is_active" id="is_active" class="form-select">
                                <option value="1" {{ old('is_active', $city->is_active) == 1 ? 'selected' : '' }}>Active (مفعل)</option>
                                <option value="0" {{ old('is_active', $city->is_active) == 0 ? 'selected' : '' }}>Inactive (غير مفعل)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-3 mt-4 pt-3 border-top border-secondary border-opacity-25">
                        <a href="{{ route('all.cities') }}" class="btn btn-outline-secondary px-4 py-2 rounded-3 fw-semibold">
                            Cancel
                        </a>
