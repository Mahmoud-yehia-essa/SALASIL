...
<style>
    /* Custom Styling for Brand Status Dropdown Select Badges */
    select.brand-status-select {
    ...
    .brand-status-select.status-badge-active {
        background-color: rgba(34, 197, 94, 0.18) !important;
        color: #22C55E !important;
        border: 1px solid rgba(34, 197, 94, 0.4) !important;
    }

    .brand-status-select.status-badge-inactive {
        background-color: rgba(148, 163, 184, 0.18) !important;
        color: #94A3B8 !important;
        border: 1px solid rgba(148, 163, 184, 0.4) !important;
    }

    .brand-status-select option {
    ...
    html.light-theme .brand-status-select option {
...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold header-title mb-1">
            <i class="bx bx-badge-check text-info me-2"></i>Truck Brands List
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Manage and view all registered truck manufacturer brands and their linked models.
        </p>
    </div>
    <div>
        <a href="{{ route('add.truck.brand') }}" class="btn btn-primary rounded-3 px-3 py-2 fw-semibold d-inline-flex align-items-center gap-2">
...
<div class="card border-0 shadow-lg rounded-4 overflow-hidden">
    <div class="card-header bg-transparent border-bottom border-secondary border-opacity-25 py-3 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <h5 class="card-title mb-0 fw-bold d-flex align-items-center gap-2">
            ...
            <span>Truck Brands Records</span>
        </h5>
                        ...
                        <tr>
                            ...
                            <td class="fw-bold fs-6">
                                <div class="d-flex align-items-center gap-2">
                                    <i class="bx bx-badge-check text-info fs-5"></i>
                            ...
                            <td>
                                <span class="badge bg-info bg-opacity-20 text-info border border-info border-opacity-50 px-3 py-1.5 rounded-pill fw-semibold">
                                    <i class="bx bx-car me-1"></i> {{ $item->models_count }} Models
                                </span>
                            </td>
                            <!-- Dynamic Status Dropdown with Instant AJAX Update -->
                            <td style="min-width: 155px;">
                                <select class="form-select brand-status-select status-badge-{{ $item->is_active ? 'active' : 'inactive' }}" 
                            ...
                            <td class="text-center" style="min-width: 100px;">
                                <div class="d-flex align-items-center justify-content-center gap-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('edit.truck.brand', $item->id) }}" 
                                    ...
                                    <!-- Delete Button -->
                                    <a href="{{ route('delete.truck.brand', $item->id) }}" 
                                       class="btn btn-sm rounded-3 p-2 d-inline-flex align-items-center justify-content-center text-white border-0 shadow-sm delete-truck-brand-btn" 
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on('change', '.brand-status-select', function(e) {
            e.preventDefault();
            var select = $(this);
            var brandId = select.attr('data-brand-id') || select.data('brand-id');
            ...
            $.ajax({
                url: "{{ route('truck.brand.status.ajax') }}",
                type: "POST",
                data: {
                    _token: csrfToken,
                    id: brandId,
                ...
                success: function(response) {
                    ...
                    if (typeof toastr !== 'undefined') {
                        toastr.success(response.message || 'Brand status updated successfully!');
                    }
                },
                error: function(xhr) {
                    select.prop('disabled', false);
                    var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'Failed to update brand status.';
