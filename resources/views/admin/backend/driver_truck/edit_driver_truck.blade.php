...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold mb-1 header-title">
            <i class="bx bx-edit-alt text-info me-2"></i>Edit Driver Truck Assignment
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Update assigned truck details, sub-types, plate number, or verification status.
        </p>
    </div>
    <div>
        ...
        <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="card-header bg-transparent border-bottom border-secondary border-opacity-25 py-3 px-4">
                <h5 class="card-title mb-0 fw-bold d-flex align-items-center gap-2">
                    ...
                    <span>Edit Assignment #{{ $driverTruck->id }}</span>
                </h5>
            </div>

            <div class="card-body p-4">
                <form method="POST" action="{{ route('update.driver.truck') }}" id="driverTruckEditForm">
                    @csrf
                    <input type="hidden" name="id" value="{{ $driverTruck->id }}">
                                ...
                                <div class="position-relative">
                                    <select name="driver_id" id="driver_id" class="form-select @error('driver_id') is-invalid @enderror" style="padding-left: 48px !important;" required>
                                        ...
                                        @foreach($drivers as $driver)
                                            <option value="{{ $driver->id }}" {{ old('driver_id', $driverTruck->driver_id) == $driver->id ? 'selected' : '' }}>
                                ...
                                <div class="position-relative">
                                    <select name="truck_type_id" id="truck_type_id" class="form-select @error('truck_type_id') is-invalid @enderror" style="padding-left: 48px !important;" required>
                                        ...
                                        @foreach($truckTypes as $type)
                                            <option value="{{ $type->id }}" {{ old('truck_type_id', $driverTruck->truck_type_id) == $type->id ? 'selected' : '' }}>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <select name="truck_sub_type_id" id="truck_sub_type_id" class="form-select @error('truck_sub_type_id') is-invalid @enderror" style="padding-left: 48px !important;">
                                        <option value="">-- None / Optional Sub-Type --</option>
                                        @foreach($subTypes as $sub)
                                            <option value="{{ $sub->id }}" {{ old('truck_sub_type_id', $driverTruck->truck_sub_type_id) == $sub->id ? 'selected' : '' }}>
                                                {{ $sub->name_en }} {{ $sub->max_payload ? '('.$sub->max_payload.' Tons)' : '' }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <span class="position-absolute text-info" style="left: 14px; top: 50%; transform: translateY(-50%); pointer-events: none; z-index: 5;">
                                ...
                                <small class="text-slate-400 d-block mt-1">
                                    Automatically updated if Main Truck Type is changed.
                                </small>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="row g-3">
                                    ...
                                    <select name="truck_brand_id" id="truck_brand_id" class="form-select @error('truck_brand_id') is-invalid @enderror" style="padding-left: 48px !important;" required>
                                        ...
                                        @if(isset($truckBrands))
                                            @foreach($truckBrands as $brand)
                                                <option value="{{ $brand->id }}" {{ old('truck_brand_id', $driverTruck->truck_brand_id) == $brand->id ? 'selected' : '' }}>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <select name="truck_model_id" id="truck_model_id" class="form-select @error('truck_model_id') is-invalid @enderror" style="padding-left: 48px !important;" required>
                                        <option value="">-- Select Brand First --</option>
                                        @if(isset($models))
                                            @foreach($models as $model)
                                                <option value="{{ $model->id }}" {{ old('truck_model_id', $driverTruck->truck_model_id) == $model->id ? 'selected' : '' }}>
                                                    {{ $model->name_en }} {{ !empty($model->name_ar) ? '('.$model->name_ar.')' : '' }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <small class="text-slate-400 d-block mt-1">
                                    Dynamic cascading dropdown: Models populate automatically based on Brand.
                                </small>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <input type="number" 
                                           ...
                                           value="{{ old('manufacturing_year', $driverTruck->manufacturing_year) }}" 
                                           required>
                                    <span class="position-absolute text-info" style="left: 14px; top: 50%; transform: translateY(-50%); pointer-events: none; z-index: 5;">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <input type="number" 
                                           ...
                                           value="{{ old('axles_count', $driverTruck->axles_count) }}" 
                                           required>
                                    <span class="position-absolute text-info" style="left: 14px; top: 50%; transform: translateY(-50%); pointer-events: none; z-index: 5;">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <input type="text" 
                                           ...
                                           value="{{ old('plate_number', $driverTruck->plate_number) }}">
                                    <span class="position-absolute text-info" style="left: 14px; top: 50%; transform: translateY(-50%); pointer-events: none; z-index: 5;">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select name="is_verified" id="is_verified" class="form-select">
                                    <option value="1" {{ old('is_verified', $driverTruck->is_verified) == 1 ? 'selected' : '' }}>Verified (Approved by Admin)</option>
                                    <option value="0" {{ old('is_verified', $driverTruck->is_verified) == 0 ? 'selected' : '' }}>Unverified (Pending Documentation)</option>
                                </select>
                            </div>
                            ...
                            <div class="col-12 mt-3">
                                <div class="form-check form-switch card p-3 border-info border-opacity-25 bg-info bg-opacity-10 rounded-3">
                                    <input class="form-check-input ms-0 me-3" type="checkbox" role="switch" id="is_default" name="is_default" value="1" {{ old('is_default', $driverTruck->is_default) == 1 ? 'checked' : '' }} style="width: 42px; height: 22px; cursor: pointer;">
...
<script type="text/javascript">
    $(document).ready(function(){
        // Store pre-selected sub-type & model ID for pre-filling on edit load
        var initialSubTypeId = "{{ old('truck_sub_type_id', $driverTruck->truck_sub_type_id) }}";
        var initialModelId   = "{{ old('truck_model_id', $driverTruck->truck_model_id) }}";
                    ...
                    success: function(data) {
                        ...
                        if (data && data.length > 0) {
                            $.each(data, function(key, value){
                                var selected = (initialSubTypeId && initialSubTypeId == value.id) ? ' selected' : '';
                                var payload = value.max_payload ? ' (' + value.max_payload + ' Tons)' : '';
                                $subTypeSelect.append('<option value="' + value.id + '"' + selected + '>' + value.name_en + payload + '</option>');
                            });
                            initialSubTypeId = null; // Clear after initial load
                        } else {
            ...
            if (brandId) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data && data.length > 0) {
                            $.each(data, function(key, value){
                                var selected = (initialModelId && initialModelId == value.id) ? ' selected' : '';
                                var nameAr = value.name_ar ? ' (' + value.name_ar + ')' : '';
                                $modelSelect.append('<option value="' + value.id + '"' + selected + '>' + value.name_en + nameAr + '</option>');
                            });
                            initialModelId = null; // Clear after initial load
                        } else {
