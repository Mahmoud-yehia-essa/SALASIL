...
/* ═════════════════════════════════════════════════════════════
   LIGHT MODE OVERRIDES FOR EDIT FIXED ROUTE PAGE
═════════════════════════════════════════════════════════════ */
...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold mb-1 header-title">
            <i class="bx bx-edit text-info me-2"></i>Edit Fixed Route #{{ $route->id }}
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Update route endpoints, quote classification, estimated distance, or status.
        </p>
    </div>
    <div>
...
<div class="rt-card">
    <div class="rt-card-header">
        ...
        <div>
            <h4 class="fw-bold text-white mb-0">Update Route #{{ $route->id }}</h4>
            <small class="text-muted">Modify route details below</small>
        </div>
    </div>
    <div class="rt-card-body">
        <form method="POST" action="{{ route('update.route') }}" id="routeForm" autocomplete="off">
            @csrf
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-6">
                    ...
                    <select name="quote_type" id="quote_type" class="form-select">
                        <option value="local" {{ $route->quote_type === 'local' ? 'selected' : '' }}>Local (Domestic)</option>
                        <option value="international" {{ $route->quote_type === 'international' ? 'selected' : '' }}>International (Cross-Border)</option>
                    </select>
                </div>
                <div class="col-12 col-md-6">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="origin_country_id" id="origin_country_id" class="form-select">
                        ...
                        @foreach($countries as $c)
                            <option value="{{ $c->id }}" {{ $route->origin_country_id == $c->id ? 'selected' : '' }}>{{ $c->name_en }} ({{ $c->name_ar }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12 col-md-6">
                    ...
                    <select name="origin_city_id" id="origin_city_id" class="form-select">
                        ...
                        @foreach($originCities as $city)
                            <option value="{{ $city->id }}" {{ $route->origin_city_id == $city->id ? 'selected' : '' }}>{{ $city->name_en }} ({{ $city->name_ar || $city->name_en }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-6">
                    ...
                    <select name="destination_country_id" id="destination_country_id" class="form-select">
                        ...
                        @foreach($countries as $c)
                            <option value="{{ $c->id }}" {{ $route->destination_country_id == $c->id ? 'selected' : '' }}>{{ $c->name_en }} ({{ $c->name_ar }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12 col-md-6">
                    ...
                    <select name="destination_city_id" id="destination_city_id" class="form-select">
                        ...
                        @foreach($destinationCities as $city)
                            <option value="{{ $city->id }}" {{ $route->destination_city_id == $city->id ? 'selected' : '' }}>{{ $city->name_en }} ({{ $city->name_ar || $city->name_en }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-6">
                    ...
                    <div class="input-group">
                        <input type="number" step="0.1" name="estimated_distance" class="form-control" placeholder="e.g. 450.5" value="{{ old('estimated_distance', $route->estimated_distance) }}">
...
$(document).ready(function() {
    ...
    $('#routeForm').on('submit', function() {
        $('#submitBtn').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span> Updating Route...');
    });
