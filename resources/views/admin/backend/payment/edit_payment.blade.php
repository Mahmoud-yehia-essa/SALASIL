...
/* ═════════════════════════════════════════════════════════════
   LIGHT MODE OVERRIDES FOR EDIT PAYMENT PAGE
═════════════════════════════════════════════════════════════ */
...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold mb-1 header-title">
            <i class="bx bx-edit text-info me-2"></i>Edit Payment Transaction #{{ $payment->id }}
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Update transaction amounts, reference ID, receipt, or payment status.
        </p>
    </div>
    <div>
...
<div class="pay-card">
    <div class="pay-card-header">
        ...
        <div>
            ...
            <small class="text-muted">Modify payment details below</small>
        </div>
    </div>
    <div class="pay-card-body">
        <form method="POST" action="{{ route('update.payment') }}" id="payForm" enctype="multipart/form-data" autocomplete="off">
            @csrf
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-8">
                    ...
                    <select name="invoice_id" id="invoice_id" class="form-select">
                        ...
                        @foreach($invoices as $inv)
                            <option value="{{ $inv->id }}" {{ $payment->invoice_id == $inv->id ? 'selected' : '' }}>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <select name="status" id="status" class="form-select">
                        <option value="completed" {{ $payment->status === 'completed' ? 'selected' : '' }}>Completed (Paid)</option>
            ...
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-4">
                    ...
                    <div class="input-group">
                        <input type="number" step="0.01" name="amount" id="amount" class="form-control" placeholder="0.00" value="{{ old('amount', $payment->amount) }}" required>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <select name="payment_method" id="payment_method" class="form-select">
                        <option value="knet" {{ $payment->payment_method === 'knet' ? 'selected' : '' }}>KNET</option>
                        <option value="credit_card" {{ $payment->payment_method === 'credit_card' ? 'selected' : '' }}>Credit Card (Visa / Mastercard)</option>
                        <option value="apple_pay" {{ $payment->payment_method === 'apple_pay' ? 'selected' : '' }}>Apple Pay</option>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <input type="text" name="transaction_id" class="form-control" placeholder="e.g. TXN-98402948" value="{{ old('transaction_id', $payment->transaction_id) }}">
                </div>
            </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    @if($payment->receipt_url)
                        <div class="mt-2" style="font-size:0.82rem;">
                            ...
                            <a href="{{ asset($payment->receipt_url) }}" target="_blank" class="text-info text-decoration-underline ms-1">View Receipt</a>
                        </div>
                    @endif
                </div>
                <div class="col-12 col-md-6">
            ...
            <div class="text-end pt-3">
                <button type="submit" class="btn btn-save-pay d-inline-flex align-items-center gap-2" id="submitBtn">
                    ...
                    <span>Update Payment Transaction</span>
                </button>
            </div>
        </form>
    </div>
...
$(document).ready(function() {
    ...
    $('#payForm').on('submit', function() {
        $('#submitBtn').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span> Updating Payment...');
    });
