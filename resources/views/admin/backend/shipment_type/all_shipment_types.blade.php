...
<style>
    ...
    select.shipment-type-status-select {
    ...
    .shipment-type-status-select.status-badge-active {
        background-color: rgba(34, 197, 94, 0.18) !important;
        color: #22C55E !important;
        border: 1px solid rgba(34, 197, 94, 0.4) !important;
    }

    .shipment-type-status-select.status-badge-inactive {
...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom border-secondary border-opacity-25 gap-3">
    <div>
        <h3 class="fw-bold header-title mb-1">
            <i class="bx bx-package text-info me-2"></i>Shipment Types
        </h3>
        <p class="text-slate-400 mb-0" style="font-size: 0.95rem;">
            Manage shipment types, multilingual titles, and active visibility status.
        </p>
    </div>
    <div>
        <a href="{{ route('add.shipment.type') }}" class="btn btn-primary rounded-3 px-3 py-2 fw-semibold d-inline-flex align-items-center gap-2">
            <i class="bx bx-plus-circle fs-5"></i>
            <span>Add New Shipment Type</span>
        </a>
    </div>
</div>
    ...
    <div class="card-body p-4">
        <div class="table-responsive">
            <table id="example" class="table table-striped table-bordered align-middle" style="width:100%">
                ...
                <tbody>
                    @foreach($shipmentTypes as $key => $item)
                    <tr>
                        <td class="text-center fw-bold">{{ $key + 1 }}</td>
                        <td class="fw-bold text-white fs-6">
                            <i class="bx bx-box me-1 text-info"></i> {{ $item->name_en }}
                        </td>
                        ...
                        <td style="min-width: 140px;">
                            <select class="form-select shipment-type-status-select status-badge-{{ $item->is_active == 1 ? 'active' : 'inactive' }}" 
                                    data-shipment-type-id="{{ $item->id }}" 
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center justify-content-center gap-2">
                                <a href="{{ route('edit.shipment.type', $item->id) }}" 
                                ...
                                <a href="{{ route('delete.shipment.type', $item->id) }}" 
                                   class="btn btn-sm btn-outline-danger rounded-3 p-2 d-inline-flex align-items-center justify-content-center delete-shipment-type-btn"
